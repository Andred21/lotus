...
| P-13 | Protótipo Figma mostra coluna **CÓDIGO** da turma com identificador próprio (`TR-45`…`TR-42`); implementado não tem coluna nenhuma | Bloco 6b (spec D7): turma se identifica por relacionamento — `quote_code` + `budget_code` (`Scap 3 - Cot 1`). Confirmado ao seedar o cenário demo (2026-07-26): as 4 turmas não têm código próprio. Criar um exigiria coluna + regra de sequência (ADR-17 fez isso para orçamento/cotação, deliberadamente não para turma). O bloco visual de refinamento de UI vai precisar decidir o que a coluna exibe | Decisão do João no planejamento do bloco visual (backlog item 2 / Notion H.1.3) |
