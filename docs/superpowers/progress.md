...
| 2026-07-21 | Bloco 6b · Sprint 3 · Turma + designação de redator (backend) | Ativo | Turma nasce por passo manual de cotação aprovada (não no approve); designação de redator separada com gate RN-09 (**REUF válido + habilitação `course_redator`**; `valid_until` nulo = vale sempre); **N:N `turma_redator`** (diverge do `redator_id` FK do der-fisico → pendência registrada). Escopo: schema + `Turma` + `RedatorIdoneidadeService` + Actions + REST tipado. Exclui import (6c) e manual/conclusão (6d) | Plano `plans/2026-07-21-bloco6b-turma-designacao.md`; `Commercial\Quote` (approved 1:1, `course_id` derivado), `Catalog` (`course_redator`), `Identity\Redator`+`RedatorDocumentType::REUF`+`files.valid_until`, morph map já tem `'turma'`, glob de rotas por domínio, lição #15 (prova MySQL), permissões `operation.turma.*` já no `PermissionCatalog` | `plans/2026-07-21-bloco6b-turma-designacao.md` | `specs/2026-07-21-bloco6b-turma-designacao-design.md` |
