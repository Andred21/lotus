...
| 2026-07-21 | Bloco 6d · Sprint 3 · Conclusão + manual (backend) | **Ativo** | Máquina de estados: `habilitada` DERIVADA (não persistida — enum vira 2 valores), conclusão terminal gated RN-16 (3 tipos de doc), blindagem RN-15 como guard, manual PDF Blade+Gotenberg. Permissões JÁ existem (`submit_docs` redator+superadmin — admin comum 403 deliberado; `complete` admin) | Plano+spec (colunas ao lado); `.claude/rules/` carrega sozinho; prova real exige MySQL + Gotenberg do compose (lição #15) | `plans/2026-07-21-bloco6d-conclusao-manual.md` | `specs/2026-07-21-bloco6d-conclusao-manual-design.md` |
