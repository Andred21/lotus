...
> para `progress-archive.md`, consultado só sob demanda. Hoje: 21 linhas.
...
| 2026-07-19 | Bloco 5.3 · Refino frontend Comercial (H.1.3) | Ativo | Fecha achados C+B das revisões código+UI da feature `commercial`: helpers de array no `useClientForm`, extrai `AddressFields`/`ContactFields`/`useBudgetDetail`, `AppDatePicker` (locale es-CL, contrato ISO anti-fuso), stepper do wizard, a11y de ícones, toolbar/ordem | Spec + plano deste bloco; rule `frontend-fsliced` (carrega sozinha); feature Comercial frontend (plano arquivado `2026-07-13`). Zero backend/schema/DTO — `generated.ts` intacto | `plans/2026-07-19-refino-comercial-frontend.md` | `specs/2026-07-19-refino-comercial-frontend-design.md` |
