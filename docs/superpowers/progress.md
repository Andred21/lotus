...
| 2026-07-20 | Bloco 5.4 · Refino frontend Administração | Ativo | Refino C+B do review de Administração: `FormErrorSummary` nos 2 diálogos (422 de permissão deixa de sumir), toolbar contextual por aba (molde CommercialPage), grupos+descrições de permissão via i18n (`permGroup.*`/`perm.*`, es-CL), campo Tipo como tag. Frontend-only | Spec+plano deste bloco; `RoleDialog`/`StaffUserDialog`/`AdministracionPage`; moldes `CommercialPage` (toolbar tabbed) e `ClientDialog` (`FormErrorSummary`); config i18n (keySeparator default `.` → chave `perm.*` com underscore); `PermissionCatalog` (fonte das 35 descrições PT). Fora: Q-1 backend (auditoria de pivot) | `plans/2026-07-20-refino-administracao-frontend.md` | `specs/2026-07-20-refino-administracao-frontend-design.md` |
