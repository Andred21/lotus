...
- Bloco 4 · CR Curso: AppTextarea + módulos reordenáveis com soma de horas (frontend) — Notion CR.2.1, CR.2.3.
  **Dois gates herdados do Bloco 3 (achados do review final):** (1) o payload do form **precisa incluir
  `modules`** — `PUT` sem o campo apaga todos os módulos do curso em silêncio (`$modules = []` + replace),
  e o `useCourseForm` de hoje manda só `{name, technical_name, description, workload_hours}`; exigir teste
  de regressão (PUT preserva módulos). (2) **key de lista React nunca pode ser `module.id`** — o replace
  troca os ids a cada save; usar índice/uuid local.
