...
> Nota: a criação de role customizada é acionada por um botão próprio. O header do `ModulePage` já tem o `AppButton` de "Nuevo usuario"; para o "Nuevo rol" há duas opções válidas — reusar o header condicionando ao tab ativo, **ou** um `AppButton` acima da `RolesTable`. Escolha o botão acima da `RolesTable` (mais simples, não acopla o header ao tab):
> ```tsx
> // em RolesTable, aceite uma prop opcional onNew e renderize o botão no topo
> ```
> Se preferir manter `RolesTable` puro, adicione o `AppButton` no `ModuleTab` antes da tabela, chamando `rolesPage.openCreate`. Implemente a variante do botão acima da tabela, com `label={t('role.new')}` e `icon="pi pi-plus"`, visível só para `canManage`.
...
- **Sem placeholder:** todo passo de código traz o código completo; a única escolha aberta (posição do botão "Nuevo rol") vem com a variante recomendada explícita. ✓
