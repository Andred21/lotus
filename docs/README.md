...
**Snapshot gerado em:** 2026-07-04 · **Atualizado em:** 2026-07-30
...
19. **Arquivo que já tem catraca própria não recebe entregável sem asserção no mesmo commit.** Duas ocorrências no `docker-compose.prod.yml`, que é guardado por `frontend/tests/compose-prod.test.ts`: primeiro um `env_file` com caminho fixo, que passava pela catraca e subiria produção com o env da sonda; depois o serviço `scheduler` do bloco de retenção, cujo sumiço deixava a suíte inteira verde com a poda parada em produção (Q-2 do review de 2026-08-28). Nos dois casos a prova foi um `docker compose config` manual, de uma vez só — que é exatamente o que a lição 14 recusa. **Antes de acrescentar serviço, chave ou propriedade a um arquivo desses, abra o teste que o guarda e some uma asserção.** Vale para os pares já existentes: `docker-compose.prod.yml`/`docker-compose.prod-probe.yml` e `compose-prod.test.ts`, `docker/nginx/prod.conf`/`deploy/nginx/tls.conf` e `nginx-conf.test.ts`, `deploy/bin/*.sh` e `backup-db.test.ts`/`verificar-backup.test.ts`, `routes.php` de domínio e `AuthenticatedRouteMiddlewareTest`, DTO com coleção nested e `PersistenceLawsTest`, `Schedule::` e `PodaAgendadaRatchetTest`. **Emenda de 2026-09-20 (Q-7 do review do item 10 v2): a lição vale para o compose E para as confs e scripts que produção monta**, não só para o compose. O `tls.conf` duplica o `prod.conf` por decisão — o bind mount troca o arquivo inteiro —, e o único guarda da igualdade era um comentário: *“Mudou o proxy lá? Muda aqui no mesmo commit.”* Ele não segurou. O `tls.conf` divergiu justamente no ponto que o healthcheck do nginx e o gate pós-deploy do `deploy.sh` consomem, o `/up` passou a responder 301 em vez de 200, e nenhum teste viu — a terceira ocorrência da mesma lição, desta vez num arquivo que ela ainda não alcançava. **Emenda de 2026-09-21 (item 12):** a lista de pares acima dizia `deploy/bin/*.sh` e `backup-db.test.ts`/`verificar-backup.test.ts`, mas **`deploy/bin/deploy.sh` não tinha asserção nenhuma** — e `.github/workflows/ci.yml` também não. Glob na lição não é catraca no arquivo. Os pares passam a ser nominais: `deploy/bin/deploy.sh` ↔ `frontend/tests/deploy-sh.test.ts`, `.github/workflows/deploy.yml` ↔ `frontend/tests/workflow-deploy.test.ts` e `deploy/aws/criar-oidc-e-role.sh` ↔ `frontend/tests/criar-oidc-role.test.ts`. O `ci.yml` segue sem catraca e isso fica **declarado**, não esquecido. **Emenda de 2026-09-26 (Q-2 do review do item 12): a catraca de um gate ancora a comparação e a saída de falha, não a palavra.** A mesma forma apareceu em dois blocos. No item 12 foram o `PROMOVER` do botão (`a9cedc39`), a contagem de concessões do `criar-oidc-role` (`7a8c3c96`) e, já no review, os gates de CI verde e de SHA na `main` e o aborto do dump. No item 28 foram três asserções do harness (`54852f7c`). As três do review continuavam verdes com o `exit 1` trocado por um aviso, ou com `|| true` na chamada, porque o nome, a URL e a variável sobrevivem ao mecanismo apagado. **Uma sonda que remove o `exit`, ou acrescenta `|| true`, tem de reprovar a catraca. Se não reprova, a asserção guarda a palavra, não o gate.** O “ver reprovar” do TDD não substitui essa sonda: ele mostra que a asserção falha sem o código novo, e não que falha com o mecanismo desligado. **Emenda de 2026-09-26 (item 31):** par nominal novo `.github/scripts/conferir-alinhamento.sh` ↔ `frontend/tests/conferir-alinhamento.test.ts` — o primeiro par da lição provado por **execução**: a catraca roda o script sobre fixtures e confere a saída e o código, então a sonda que troca o `exit 1` por `exit 0` reprova sem depender de nenhuma palavra do fonte.
