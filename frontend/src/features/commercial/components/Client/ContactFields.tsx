          ...
          <div className="space-y-3 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                {c.is_primary && (
                  <AppTag
                    value={t("client.contactPrimaryTag")}
                    severity="info"
                  />
                )}
              </label>
              ...
              {!readOnly && (
                <span
                  title={
                    isLast
                      ? t("client.lastContactHint")
                      : t("client.removeContact")
                  }
                >
                  <AppButton
                    icon="pi pi-trash"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <FormField
                ...
                error={fieldErrors?.[`contacts.${i}.name`]?.[0]}
              >
                <AppInputText
                  value={c.name}
                  disabled={readOnly}
              ...
              <FormField
                ...
                error={fieldErrors?.[`contacts.${i}.job_title`]?.[0]}
              >
                <AppInputText
                  value={c.job_title ?? ""}
                  disabled={readOnly}
                  onChange={(e) => onPatch(i, { job_title: e.target.value })}
                  className="w-full"
                />
              </FormField>
              <FormField
                label={t("common.email")}
                error={fieldErrors?.[`contacts.${i}.email`]?.[0]}
              >
                <AppInputText
                  value={c.email ?? ""}
                  disabled={readOnly}
              ...
              <FormField
                ...
                error={fieldErrors?.[`contacts.${i}.phone`]?.[0]}
              >
                <AppInputText
                  value={c.phone ?? ""}
                  disabled={readOnly}
